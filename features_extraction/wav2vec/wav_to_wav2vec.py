...
def main(args):
    ...
    #wav2vec encoder
    encoder = wavencoder.models.Wav2Vec(pretrained=True).eval().to(device)
    ...
    for row,col in tqdm(df.iterrows()):
        ...
        #read wav file
        x, _ = torchaudio.load(wav_name)
        x = x.to(device)
    
        #encode
        with torch.no_grad():
            out = encoder(x)
        wav2vec = np.asarray(torchvision.transforms.functional.to_pil_image(out, mode='L'))
        wav2vec_len = wav2vec.shape[1]
        ...
        #segment
        wav2vec_seg = np.expand_dims(wav2vec, 0).transpose(0,2,1) #(1,T,F)
        ...
        if num_segments == 1:   #utterance is <= 3 sec
            ...
            if wav2vec_len > 450:
                pil_img = Image.fromarray(wav2vec[:,150:150+segmentsize])
                pil_img.save(f'{trainimgpath}{label}/{utt}_x1.png')
                nutt += 1
                
                pil_img = Image.fromarray(wav2vec)
                cropped = torchvision.transforms.functional.center_crop(pil_img, output_size=(512,segmentsize))
    ...
    for row,col in tqdm(df.iterrows()):
        ...
        im = torchvision.transforms.functional.to_pil_image(x, mode='L')
        im.save(devimgpath+utt+'.png')
        nutt += 1
